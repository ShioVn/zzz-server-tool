<script lang="ts">
	...
	// Map weapon config index → agent info (for circular icon badges)
	// Each agent's weaponId points to a weapon config entry by weapon numeric id.
	// We assign each agent to the first unclaimed config entry with that weapon id.
	const weaponEntryAgentMap = $derived.by<
		Map<number, { name: string; icon?: string }>
	>(() => {
		const map = new Map<number, { name: string; icon?: string }>();
		const claimed = new Set<number>();
		for (const ao of app.zonConfig.avatarOverrides) {
			if (!ao.weaponId) continue;
			const agent = agents.find(
				(a) => a.zonEnum === ao.id || String(a.id) === ao.id,
			);
			if (!agent) continue;
			const info = { name: agent.name ?? "?", icon: agent.icon };
			// Find first unclaimed config entry with this weapon id
			for (let i = 0; i < app.zonConfig.configWeapons.length; i++) {
				if (Number(app.zonConfig.configWeapons[i].id) !== ao.weaponId)
					continue;
	...
	const filterOptions = $derived({
		...
		subs: [
			...new Set(
				weapons.map((w) => w.sub).filter((v): v is string => v != null),
			),
		].sort(),
	});
	...
	function getTypeIcon(val: number): string {
		return WEAPON_TYPE_ICONS[val]
			? getAssetUrl(WEAPON_TYPE_ICONS[val])
			: "";
	}
	...
	async function saveWeapon() {
		...
		try {
			...
			await runControlCommand("create-weapon", [uid, String(modalWeapon.id), String(modalLevel), "5", String(modalRefine)]);
			showToast(
				t(editIndex != null ? "toast.weaponEditSaved" : "toast.weaponAdded"),
				"success",
			);
		} catch (err) {
			showToast("Failed to apply weapon to server: " + String(err), "error");
		}
		closeModal();
	}
	...
	function addWeaponById() {
		...
		if (!id) {
			showToast(t("toast.weaponInvalidId"), "error");
			return;
		}
...
<div class="weapons-page">
	...
	<div class="flex items-center gap-3 mb-3">
		...
		<div class="flex items-center gap-2 shrink-0">
			<input
				...
				onkeydown={(e) => {
					if (e.key === "Enter") addWeaponById();
				}}
			/>
			<button
				class="primary-btn !py-2.5 !px-4 text-sm"
				onclick={addWeaponById}
				title="Add by ID"
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
	...
	{#if app.zonConfig.configWeapons.length > 0}
		{@const sortedWeapons = app.zonConfig.configWeapons
			...
			.sort((a, b) => {
				const rankA =
					Number(a.entry.id) >= 14000
						? 3
						: Number(a.entry.id) >= 13000
							? 2
							: 1;
				const rankB =
					Number(b.entry.id) >= 14000
						? 3
						: Number(b.entry.id) >= 13000
							? 2
							: 1;
		...
		<section>
			<h3 class="text-sm font-semibold text-white mb-3">
				...
				<span class="text-xs font-normal text-slate-500 ml-2"
					>({app.zonConfig.configWeapons.length})</span
				>
			</h3>
			<div class="weapon-inv-grid">
				{#each sortedWeapons as { entry, originalIndex }}
					...
					{@const eqAgent = weaponEntryAgentMap.get(originalIndex)}
					<button
					...
					>
						...
						{#if eqAgent}
							<div class="weapon-inv-agent-badge">
								{#if eqAgent.icon}
									<img
										src={eqAgent.icon}
										alt={eqAgent.name}
										class="weapon-inv-agent-icon"
										loading="lazy"
									/>
								{:else}
									<span class="weapon-inv-agent-initial"
										>{eqAgent.name.charAt(0)}</span
									>
								{/if}
							</div>
						{/if}
						<div
							class="weapon-inv-rank"
							class:rank-s={numId >= 14000}
							class:rank-a={numId >= 13000 && numId < 14000}
						>
							{rankLabel(numId)}
						...
						<div class="weapon-inv-icon-wrap">
							{#if icon}
								<img
									src={icon}
									alt={getWeaponName(entry.id)}
									class="weapon-inv-icon"
									loading="lazy"
								/>
							{:else}
								<div class="weapon-inv-icon-fallback">
									{getWeaponName(entry.id).charAt(0)}
								</div>
							{/if}
						</div>
						<div class="weapon-inv-info">
							<p class="weapon-inv-name">
								{getWeaponName(entry.id)}
							</p>
							<div class="weapon-inv-meta">
								<span class="weapon-inv-level"
									>Lv.{entry.level ?? 60}</span
								>
								<span class="weapon-inv-refine"
									>R{entry.refine ?? 1}</span
								>
							</div>
						</div>
		...
		>
			...
			<div class="catalog-filters">
				<div class="catalog-filter-row">
					<span class="catalog-filter-label"
						>{t("weapon.filter.type")}</span
					>
				...
				<div class="catalog-filter-row">
					...
					<div class="catalog-filter-chips">
						{#each filterOptions.ranks as val}
							...
							>
								<img
									src={getRankIconSvg(val >= 4)}
									alt=""
									class="h-4 w-4 shrink-0 rounded-sm"
								/>
								<span class="font-bold"
									>{getRankLabel(val)}</span
								>
							</button>
						{/each}
					</div>
				...
				<div class="catalog-filter-row">
					...
					<div class="catalog-filter-chips">
						...
						{#if hasActiveFilters()}
							<button
								class="chip-filter !border-rose-500/30 !text-rose-300 hover:!border-rose-500/50"
				...
				{:else}
					<div class="weapon-catalog-grid">
						{#each filteredWeapons as weapon (weapon.id)}
							...
							>
								<div
									class="weapon-catalog-rank"
									class:rank-s={numId >= 14000}
									class:rank-a={numId >= 13000 &&
										numId < 14000}
								>
									{rankLabel(numId)}
								</div>
								{#if weapon.icon}
									<img
										src={weapon.icon}
										alt={weapon.name ?? String(weapon.id)}
										class="weapon-catalog-icon"
										loading="lazy"
									/>
								{:else}
									<div class="weapon-catalog-icon-fallback">
										?
									</div>
								{/if}
								<div class="weapon-catalog-info">
									<p class="weapon-catalog-name">
										{weapon.name ?? `ID ${weapon.id}`}
									</p>
									<p class="weapon-catalog-id">
										ID: {weapon.id}
									</p>
								</div>
							</button>
...
{#if modalWeapon}
	...
	>
		...
		>
			...
			<div
				class="modal-header"
				style="--accent: {Number(modalWeapon.id) >= 14000
					? '#fbbf24'
					: '#a78bfa'}"
			>
				<div class="modal-icon-wrap">
					{#if modalWeapon.icon}
						<img
							src={modalWeapon.icon}
							alt={modalWeapon.name ?? ""}
							class="modal-icon weapon-modal-icon"
						/>
					{:else}
						<div class="modal-icon-fallback">
							{(modalWeapon.name ?? "?").charAt(0)}
						</div>
					{/if}
					<div
						class="weapon-modal-rank"
						class:rank-s={Number(modalWeapon.id) >= 14000}
						class:rank-a={Number(modalWeapon.id) >= 13000 &&
							Number(modalWeapon.id) < 14000}
					>
				...
				<div class="modal-title-group">
					<h2 class="modal-name">
						{modalWeapon.name ?? `Weapon #${modalWeapon.id}`}
					</h2>
					<p class="modal-subtitle">
						ID: {modalWeapon.id} &middot; {editIndex != null
							? "Editing"
							: "Adding"}
					</p>
				</div>
			...
			<div class="modal-body">
				<div class="modal-row-2">
					<div class="field-group">
						<label for="weapon-modal-level" class="field-label"
							>{t("weapon.level")}</label
						>
						<select
							id="weapon-modal-level"
							class="field-input modal-select"
							value={modalLevel}
							onchange={(e) =>
								(modalLevel = Number(
					...
					<div class="field-group">
						<label for="weapon-modal-refine" class="field-label"
							>{t("weapon.refine")}</label
						>
						<select
							id="weapon-modal-refine"
							class="field-input modal-select"
							value={modalRefine}
							onchange={(e) =>
								(modalRefine = Number(
			...
			<div class="modal-footer">
				{#if editIndex != null}
					<button
						...
						onclick={() => removeWeapon(editIndex!)}
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							viewBox="0 0 20 20"
...
<style>
	...
	.weapon-catalog-rank.rank-s {
		background: linear-gradient(135deg, #fbbf24, #f59e0b);
		color: #1e1b0a;
	}
	.weapon-catalog-rank.rank-a {
	...
	.weapon-inv-rank.rank-s {
		background: linear-gradient(135deg, #fbbf24, #f59e0b);
		color: #1e1b0a;
	}
	.weapon-inv-rank.rank-a {
		background: linear-gradient(135deg, #a78bfa, #8b5cf6);
		color: #1e1b4b;
	}
	.weapon-inv-icon-wrap {
		width: 48px;
		height: 48px;
		border-radius: 12px;
	...
	.weapon-inv-card:hover .weapon-inv-remove {
		opacity: 1;
	}
	.weapon-inv-agent-badge {
		position: absolute;
		top: -20px;
		left: -20px;
		width: 60px;
	...
	.weapon-modal-rank.rank-s {
		background: linear-gradient(135deg, #fbbf24, #f59e0b);
		color: #1e1b0a;
	}
	.weapon-modal-rank.rank-a {
		background: linear-gradient(135deg, #a78bfa, #8b5cf6);
		color: #1e1b4b;
	}
	.weapon-modal-icon {
	...
	:global(.catalog-popup-title-icon) {
		width: 20px;
		height: 20px;
		color: #22d3ee;
	}
	:global(.catalog-popup-close) {
		width: 32px;
		height: 32px;
		border-radius: 50%;
		border: none;
	...
	:global(.catalog-popup-close:hover) {
		background: rgba(239, 68, 68, 0.25);
		color: #fca5a5;
	}
	:global(.catalog-popup-search) {
		position: relative;
		padding: 0.75rem 1.25rem;
		flex-shrink: 0;
	}
	:global(.catalog-popup-search-icon) {
		position: absolute;
		left: 1.75rem;
		top: 50%;
	...
	:global(.catalog-popup-search-input) {
		width: 100%;
	...
	:global(.catalog-popup-search-input:focus) {
		border-color: rgba(34, 211, 238, 0.6);
		box-shadow: 0 0 0 2px rgba(34, 211, 238, 0.12);
	}
	:global(.catalog-popup-body) {
	...
	:global(.footer-btn.primary-btn) {
		background: rgba(34, 211, 238, 0.15);
		border: 1px solid rgba(34, 211, 238, 0.3);
		color: #a5f3fc;
	}
	:global(.footer-btn.primary-btn:hover) {
